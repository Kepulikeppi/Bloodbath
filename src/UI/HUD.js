...
export class HUD {
    constructor() {
        ...
        this.weaponCaliber = document.getElementById('val-weapon-caliber'); // NEW
    ...
    update() {
        ...
        if (wConf) {
            ...
            this.weaponCaliber.innerText = wConf.caliber || ""; // NEW
        ...
        for(let i=1; i<=6; i++) {
            ...
            if (!el) continue;
            const status = state.getSlotStatus(i);
            el.className = 'slot';
            if (status === 'active') el.classList.add('active');
            else if (status === 'owned') el.classList.add('owned');
