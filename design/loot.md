#Ideas for possible loot items. 

#Ammunition types
- 9mm pistol
- .44 magnum pistol
- shotgun shells (shot and slugs)
- 7.62mm
- 12.7mm
- 40mm grenade (for a launcher)

#Ammunition drop sizes
- tiny (3 to 5 rounds)
- small (one full magazine ~20 rounds)
- medium (three full magazines ~40 rounds)
- large (ten full magazines ~200 rounds)
- epic (thirty full magazines ~600 rounds)

#Boosts/consumables 
- health packs
- XP packs
- rage pills

#Crafting materials
- metal scraps
- electronic parts
- microchips

#Special
- batteries (will power some weapons/tools)

#Ranged weapon drops (unlocks a weapon and provides a medium ammo drop for that weapon)
- 9mm pistol (semi-auto)
- .44 revolver
- double-barrel shotgun (manual reload)
- semi-auto shotgun
- light MG (belt-fed)
- heavy MG (belt-fed)
- 40mm grenade launcher
- electronic stun gun (allows hacking mechanical enemies to make them an ally)

#Melee weapon drops (unlocks a weapon)
- Nordic shortsword
- birch log
- handheld circular saw
- billhook
